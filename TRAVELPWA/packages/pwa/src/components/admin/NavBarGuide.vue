        ...
        <div>
          <div class="flex flex-col w-full gap-4">
            ...
            >
              ...
              <div
                v-else
                class="w-40px h-40px flex justify-center items-center bg-gray-100 border-2 border-gray-200 border-rd-[50%] flex"
              >
                <img
                  src="/lama.svg"
                  alt="User Profile Image"
...
const closeProfileSettings = async () => {
  ...
  console.log(firebaseUser.value?.photoURL)
}
...
const links = ref([
  { to: '/guide/dashboard', text: 'Dashboard', icon: Home },
  {
    to: '/guide/reports',
    text: 'Reports',
    icon: NotebookPen,
...
// Ensure firebaseUser is updated reactively by forcing Vue to notice changes

// When restoring the user, make sure that `firebaseUser` updates and triggers the computed properties
