<template>
  <div class="flex gap-2 w-full cursor-pointer group" @click="$emit('click')">
    <div
      class="group-hover:shadow-md p-4 border-b bg-gray-100 border-gray-200 group-hover:bg-gray-200 w-2/3 my-2 rounded-md flex justify-between items-center"
    >
      <div>
        <div>
          <p class="font-semibold">{{ activity?.activity?.name }}</p>
          <p class="text-sm my-2">{{ activity?.activity?.description }}</p>
        </div>
