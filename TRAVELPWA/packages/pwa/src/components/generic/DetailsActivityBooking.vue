  ...
  >
    ...
    >
      ...
      >
        ...
        <img
          :src="booking.activity.headerImageUrl"
          alt="Activity Header"
        ...
        <div class="absolute bottom-2 left-2 flex gap-2 items-center">
          ...
          <div
            v-if="booking.activity.recommended"
      ...
      <p class="pl-2 text-gray-700 text-sm sm:text-base mb-6">
        <strong>Age Group:</strong> {{ booking.activity.ageGroup }}
      </p>
      <!-- Details Grid -->
      <div class="mb-6">
        <div class="bg-gray-100 p-4 rounded-md">
          <div class="w-full bg-white p-4 rounded-md">
            <p class="text-gray-700 text-sm sm:text-base">
              <strong>Date:</strong> {{ formatFullDate(booking.startDate) }}
            </p>
            <p class="text-gray-700 text-sm sm:text-base">
              <strong>Time:</strong> {{ formatTime(booking.startDate) }}
            </p>
            <p class="text-gray-700 text-sm sm:text-base">
              <strong>Duration:</strong> {{ booking.activity.duration }} hours
            </p>
          </div>
        </div>
        <!-- Equipment Provided -->
        <div
          v-if="
            booking.activity.equipmentProvided &&
            booking.activity.equipmentProvided.length
          "
        ...
        >
          ...
          <div class="flex flex-wrap gap-2">
            <span
              v-for="item in booking.activity.equipmentProvided"
              :key="item"
        ...
        <div
          v-if="
            booking.activity.safetyMeasures &&
            booking.activity.safetyMeasures.length
          "
          class="pl-2 col-span-2 mt-4"
        >
          <p class="text-gray-700 mb-2 text-sm sm:text-base">
          ...
          <div class="flex flex-wrap gap-2">
            <span
              v-for="item in booking.activity.safetyMeasures"
              :key="item"
              class="bg-amber-100 text-amber-700 px-3 py-1 rounded-full shadow-sm"
            >
              {{ item }}
